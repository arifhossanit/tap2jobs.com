document.addEventListener('DOMContentLoaded', loadBlogCategoryData);
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                (result.data.unlimited_plan == 1) ? $('#editUnlimitedPlan').
                    prop('checked', true) : $('#editUnlimitedPlan').
                    prop('checked', false);
...
listenChange('#add_unlimited_plan', function (event) {
    event.preventDefault();
    let status = $(this).is(':checked');
    if (status) {
        $('#allowedJobs').val(99999);
    }else{
        $('#allowedJobs').val('');
    }
});

listenChange('#editUnlimitedPlan', function (event) {
    event.preventDefault();
    let status = $(this).is(':checked');
    if (status) {
        $('#editAllowedJobs').val(99999);
    }else{
        $('#editAllowedJobs').val('');
    }
});
