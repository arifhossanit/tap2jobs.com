...
function initJobSearchMobileFilterToggle() {
    let toggle = document.querySelector(".find-jobs-filter-mobile-toggle");
    let panel = document.querySelector("#findJobsFilter");
    ...
    toggle.addEventListener("click", function (event) {
        ...
        let isOpen = panel.classList.toggle("is-open");
...
function loadJobSearchData() {
    ...
    $(".find-jobs-filter__form")
        .off("submit.jobSearch")
        .on("submit.jobSearch", function (event) {
            event.preventDefault();
        });

    let input = $("#input").val() ? JSON.parse($("#input").val()) : {};
    let currentLanguage =
        typeof lancode !== "undefined"
            ? lancode
    ...
    if (jobExperienceSlider.length) {
        let maximumExperience = Math.max(
            1,
            Number(jobExperienceSlider.data("max")) || 30,
        );
        let hasExperienceRange =
            Object.prototype.hasOwnProperty.call(input, "jobExperienceFrom") ||
            Object.prototype.hasOwnProperty.call(input, "jobExperienceTo");
        let legacyExperience = input.jobExperience ?? input.experience;
        let selectedExperienceFrom = hasExperienceRange
            ? Number(input.jobExperienceFrom || 0)
            : 0;
        let selectedExperienceTo = hasExperienceRange
            ? Number(input.jobExperienceTo || maximumExperience)
            : legacyExperience !== undefined
              ? Number(legacyExperience)
              : maximumExperience;
        selectedExperienceFrom = Math.min(
            Math.max(0, selectedExperienceFrom),
            maximumExperience,
        );
        selectedExperienceTo = Math.min(
            Math.max(selectedExperienceFrom, selectedExperienceTo),
            maximumExperience,
        );
...
document.addEventListener("livewire:load", function () {
    $(document)
        .off("click.jobSearchResults", "#jobsSearchResults ul li")
        .on("click.jobSearchResults", "#jobsSearchResults ul li", function () {
