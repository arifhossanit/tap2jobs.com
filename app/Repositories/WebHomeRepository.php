...
class WebHomeRepository
{
    ...
    public function getDataCounts()
    {
        ...
        $data['jobs'] = Job::whereStatus(Job::STATUS_OPEN)->where('status', '!=',Job::STATUS_DRAFT)->whereIsSuspended(Job::NOT_SUSPENDED)->whereDate('job_expiry_date', '>=', Carbon::now()->toDateString())->count();
        $data['vacancies'] = Job::whereStatus(Job::STATUS_OPEN)->where('status', '!=',Job::STATUS_DRAFT)->whereIsSuspended(Job::NOT_SUSPENDED)->whereDate('job_expiry_date', '>=', Carbon::now()->toDateString())->sum('vacancy');
    ...
    public function jobSearch($searchTerm): array
    {
        if ($searchTerm) {
            $jobSearchResult = Job::with([
                ...
                ->whereDate('job_expiry_date', '>=', Carbon::now()->toDateString())->get();
